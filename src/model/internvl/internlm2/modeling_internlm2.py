...
# `generate()` lives on `GenerationMixin` in recent Transformers versions.
# InternLM2 defines `prepare_inputs_for_generation`, so we explicitly include
# `GenerationMixin` on the causal LM class to keep `.generate()` working and to
# ensure `generation_config` is created (via `can_generate()`).
