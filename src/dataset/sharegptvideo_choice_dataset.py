...
class ShareGPTVideoChoiceDataset(Dataset):
    def __init__(
        self,
        base_path: str,
        jsonl_path: str,
        ...
        local_rank: int = 0,
        train: bool = True,
        question_instruction: str = "Please provide your options (only A or B or C or D) directly. Do not give any other responses.",
        options_list: List[str] = ["A", "B", "C", "D"],
        ...
        self.train = train
        self.question_instruction = question_instruction
        ...
        self.options_token_ids = [self.tokenizer.convert_tokens_to_ids(t) for t in self.options_list]
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    if ("question" not in annotation) or ("answer" not in annotation) or ("options" not in annotation):
                        skipped_count += 1
                        continue
                    # Check "question", "answer" and "options" is NoneType
                    if annotation["question"] is None or annotation["answer"] is None or annotation["options"] is None:
                        skipped_count += 1
                        continue
                    # Check "question" "answer" and "options" is non-empty
                    if len(annotation["question"].strip()) == 0 or len(annotation["answer"].strip()) == 0 or len(annotation["options"]) == 0:
                        skipped_count += 1
                        continue
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        for key in sorted(options.keys()):
            raw_q += f"\n{key}. {options[key]}"
        raw_q += f"\n{self.question_instruction}"

        data_type = ann.get("data_type", "video")
        vision_messages = self._build_messages(raw_q, vision_path, data_type)

        # Text prompt: Qwen uses the multimodal processor chat template; others use the tokenizer chat template.
        if self.prompt_style == "qwen2_5_vl":
            ...
            qa_messages = vision_messages + [{"role": "assistant", "content": ann["answer"]}]
            ...
            qa_messages = text_messages + [{"role": "assistant", "content": ann["answer"]}]
            question_and_answer = self.tokenizer.apply_chat_template(
                qa_messages, tokenize=False, add_generation_prompt=False
            )
            # Chat template already contains BOS/EOS control tokens.
...
if __name__ == '__main__':
    ...
    base_path = "/data/siyou/QTSplus/datasets/ShareGPTVideo_train/data/train_300k"
    jsonl_path = "/data/siyou/QTSplus/datasets/ShareGPTVideoChoice/choice_eval.jsonl"
    processor = Qwen2_5_VLVisionProcessor.from_pretrained("/data/siyou/QTSplus/pretrained_models/Qwen2.5-VL-3B-Instruct-Vision")
    dataset = ShareGPTVideoChoiceDataset(
        base_path = base_path,
