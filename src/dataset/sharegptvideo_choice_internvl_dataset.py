"""
InternVL2.5 (image) multiple-choice dataset (VSCQ-style) for QTSplus training.
...
The prompt is formatted using InternVL's image tokens:
  - `<img> ... </img>` wrapper
...
class ShareGPTVideoChoiceInternVLDataset(Dataset):
    def __init__(
        self,
        base_path: str,
        jsonl_path: str,
        ...
        local_rank: int = 0,
        train: bool = True,
        question_instruction: str = "Please provide your options and provide additional details.",
    ...
    def _load_jsonl(path: str, base_path: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                ...
                if ("question" not in ann) or ("answer" not in ann) or ("options" not in ann):
                    skipped += 1
                    continue
                if ann["question"] is None or ann["answer"] is None or ann["options"] is None:
                    skipped += 1
                    continue
                if len(str(ann["question"]).strip()) == 0 or len(str(ann["answer"]).strip()) == 0:
                    skipped += 1
                    continue
                if not isinstance(ann["options"], dict) or len(ann["options"]) == 0:
                    skipped += 1
                    continue
